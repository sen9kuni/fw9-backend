...
exports.getAllNotifs = (id, sort_by, cb) => {
  db.query(`SELECT notification.id, notification.created_at, trans_id, is_read, t1.amount, t3.name type, t2.user_id receiverId, t2.first_name receiverFirstName, t2.last_name receiverLastName, t2.picture imgReceiver, t2.phonenumber receiverPhonenumber, t4.first_name senderFirstName, t4.last_name senderLastName, t4.picture imgSender FROM notification FULL OUTER JOIN transactions t1 on t1.id = notification.trans_id FULL OUTER JOIN transaction_type t3 on t3.id = t1.type_id_trans FULL OUTER JOIN profile t2 on t2.user_id = t1.recipient_id FULL OUTER JOIN profile t4 on t4.user_id = t1.sender_id WHERE notification.user_id= ${id} AND notification.is_read = FALSE ORDER BY created_at ${sort_by}`, (err, res) => {
    cb(err, res.rows);
  });
};
